...
export default function TorneoDetailPage() {
  ...
  if (!torneo) {
    return (
      <div className="p-6 lg:p-8 max-w-5xl mx-auto text-center py-24 text-tinta-3">
  ...
  return (
    <div className="p-6 lg:p-8 max-w-5xl mx-auto">
      <Link href="/admin/torneos" className="inline-flex items-center gap-2 text-sm text-tinta-3 hover:text-white mb-6 transition-colors">
        <ArrowLeft size={16} /> Mis torneos
      </Link>

      <div className="relative rounded-[6px] overflow-hidden mb-6 h-36 bg-acento">
        <div className="absolute inset-0 bg-fondo/85" />
        <div className="relative p-6 h-full flex flex-col justify-end">
          <div className="flex items-end justify-between flex-wrap gap-3">
            <div>
              <h1 className="text-2xl font-bold text-white">{torneo.nombre}</h1>
              <span className="text-xs text-tinta-3">{ediciones.length} edición(es)</span>
            </div>
            <div className="flex gap-2">
              {ediciones[0] && (
                <Link
                  href={`/torneos/${ediciones[0].slug}`}
                  className="flex items-center gap-1.5 px-3 py-2 bg-white/10 hover:bg-white/20 text-white text-xs rounded-[6px] border border-white/20 transition-all"
                >
              ...
              <Link
                ...
                className="flex items-center gap-1.5 px-3 py-2 bg-acento hover:bg-acento text-white text-xs font-semibold rounded-[6px] transition-all"
              >
      ...
      <div className="flex gap-1 mb-6 bg-white/5 p-1 rounded-[6px] w-fit">
        {([
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-[4px] text-sm font-medium transition-all ${activeTab === tab.id ? 'bg-acento text-white' : 'text-tinta-3 hover:text-white'}`}
          >
            {tab.icon} {tab.label}
          </button>
        ))}
      ...
      {activeTab === 'ediciones' && (
        <div className="space-y-4">
                ...
                <div className="flex items-start justify-between mb-4 flex-wrap gap-2">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-xs font-bold text-tinta-4 uppercase">Edición #{ed.numero}</span>
                  ...
                  <div className="flex items-center gap-2">
                    {ed.bolsa_premios && <div className="text-sm font-bold text-atencion">{ed.bolsa_premios}</div>}
                    <select
                      value={ed.estado}
                      ...
                      className="bg-white/5 border border-borde text-tinta-2 text-xs rounded-[4px] px-2 py-1.5 focus:outline-none focus:border-acento disabled:opacity-50"
                    >
                      {ESTADOS_DISPONIBLES.map(o => (
                        <option key={o.value} value={o.value} className="bg-superficie text-white">{o.label}</option>
                      ))}
                    </select>
                    {cambiandoEstado === ed.id && <Loader2 size={14} className="animate-spin text-tinta-3" />}
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div className="text-center p-3 bg-white/5 rounded-[6px]">
                    <p className="text-lg font-bold text-white">{ed.equipos_aprobados}</p>
                    <p className="text-xs text-tinta-3">{ed.max_equipos ? `/ ${ed.max_equipos} equipos` : 'equipos aprobados'}</p>
                  </div>
                  <div className="text-center p-3 bg-white/5 rounded-[6px]">
                    <p className="text-sm font-bold text-white">
                      {ed.fecha_inicio ? new Date(ed.fecha_inicio).toLocaleDateString('es', { day: 'numeric', month: 'short' }) : '—'}
                    </p>
                    <p className="text-xs text-tinta-3">Inicio</p>
                  </div>
                </div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {[
                    ...
                    <Link
                      key={link.href}
                      href={link.href}
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-white/5 hover:bg-white/10 border border-borde hover:border-white/25 text-tinta-2 hover:text-white text-xs rounded-[6px] transition-all"
                    >
          ...
          <Link
            href={`/admin/torneos/${torneoId}/ediciones/nueva`}
            className="flex items-center justify-center gap-2 p-5 bg-superficie border-2 border-dashed border-borde rounded-[6px] hover:border-borde text-tinta-4 hover:text-acento-claro transition-all text-sm"
          >
      ...
      {activeTab === 'config' && (
        <div className="glass-card p-6 space-y-4">
          ...
          <div>
            ...
            <div className="w-full bg-white/5 border border-borde text-white rounded-[6px] px-4 py-3 text-sm">{torneo.nombre}</div>
          </div>
          {torneo.descripcion && (
            <div>
              <label className="block text-xs font-semibold text-tinta-3 uppercase tracking-wider mb-2">Descripción</label>
              <div className="w-full bg-white/5 border border-borde text-tinta-2 rounded-[6px] px-4 py-3 text-sm">{torneo.descripcion}</div>
            </div>
          )}
          <p className="text-xs text-tinta-4">Editar estos datos todavía no está disponible.</p>
        </div>
      )}

      {activeTab === 'config' && (
        <div className="mt-4 bg-rose-950/10 border border-rose-500/20 rounded-[6px] p-6 space-y-4">
          <div className="flex items-center gap-2 text-vivo">
          ...
          <p className="text-xs text-tinta-3">
            ...
            en sí no se borran, siguen existiendo para otros torneos. <strong className="text-vivo">Esta acción no se puede deshacer.</strong>
          </p>
          {deleteError && (
          ...
          <div>
            <label className="block text-xs font-semibold text-tinta-3 mb-2">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-white/5 border border-borde text-white rounded-[6px] px-4 py-2.5 text-sm focus:outline-none focus:border-rose-500"
              />
              <button
                type="button"
                onClick={handleEliminarTorneo}
                disabled={confirmarNombre !== torneo.nombre || eliminando}
                className="flex items-center gap-2 px-4 py-2.5 bg-rose-600 hover:bg-rose-500 disabled:bg-white/5 disabled:text-tinta-4 text-white text-xs font-bold rounded-[6px] transition-all disabled:cursor-not-allowed"
              >
