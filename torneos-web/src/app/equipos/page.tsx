  ...
  return (
    <div className="min-h-screen flex flex-col bg-fondo text-tinta selection:bg-acento selection:text-white">
      ...
      <main className="flex-1 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 w-full space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-borde pb-6">
          <div>
            <h1 className="text-2xl sm:text-3xl font-black text-white flex items-center gap-2.5">
              <Users className="text-acento-claro" /> Equipos
            </h1>
            <p className="text-xs text-tinta-3 mt-1">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-8 pr-3 py-2.5 glass-card text-xs text-white focus:outline-none focus:border-acento w-full sm:w-64"
            />
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-superficie border border-rose-500/20 rounded-[8px] p-10 text-center text-sm text-tinta-3">
            No pudimos conectar con el servidor.
          </div>
        ) : equipos.length === 0 ? (
          <div className="bg-superficie border border-borde rounded-[8px] p-12 text-center text-sm text-tinta-3">
            {buscar
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {equipos.map(e => (
              <Link
                key={e.id}
                href={`/equipos/${e.id}`}
                className="bg-superficie border border-borde hover:border-acento/50 rounded-[6px] p-4 flex items-center gap-3 group transition-all"
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="font-bold text-sm text-white truncate group-hover:text-acento-claro transition-colors">
                    {e.nombre}
                  </p>
                  <span className="text-[11px] text-tinta-3 flex items-center gap-2">
                    <span>{e.torneos_jugados} {e.torneos_jugados === 1 ? 'torneo' : 'torneos'}</span>
                    {e.partidas_ganadas > 0 && (
                      <span className="flex items-center gap-1 text-ok/70">
                        <Trophy size={10} /> {e.partidas_ganadas}
                      </span>
                    )}
                  </span>
                </div>
                <ChevronRight size={15} className="text-tinta-4 group-hover:text-acento-claro transition-colors shrink-0" />
              </Link>
            ))}
          </div>
        )}
